...
from MOPFrame import MOPFrame


class MOPExampleFrame(tk.Frame):
      """
         | Unifica dos elementos: Canvas y MOPFrame. La razón de esto es que, en promedio la
      ...
      def __init__(self,parent,features): 
          ...
          #Se crea el Frame que contiene los M.O.P.'s disponibles junto con sus parámetros
          #(véase Controller/XML/MOPExamples.xml).
          self.__mop_frame = MOPFrame(self.__canvas,parent,features) 
          ...
          self.__mop_frame.bind("<Configure>",self.__update_scrollbar)
          self.__mop_frame.pack(side = tk.TOP,fill = tk.X,expand = True)

          #Esta instrucción le indica al Canvas que creé una ventana en donde se almacenará el ContentFrame.
          self.__canvas.create_window((0,0),window = self.__mop_frame,anchor = tk.NW)
